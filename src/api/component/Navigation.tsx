...
const Navigation = ({ isLoggedIn, setIsLoggedIn }: loginState) => {
  ...
  const onSearchButtonClick = () => {
    if (KEAWORD == "") {
      ...
      axios({
        method: "GET",
        url: `/api/board/search?name=${KEAWORD}`,
      })
        .then((res) => {
          console.log(res);
          setCurrentpage(1);
  ...
  const onLogoutClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    axios({
      ...
      .then((res) => {
        ...
        navigate("/api/main", { replace: true });
      })
      .catch((err) => {
  ...
  return (
    <nav className="nav">
      ...
      <div className="nav-board">
        <Link to="/api/board">전통주 둘러보기</Link>
      </div>
      <div className="nav-test">
        <Link to="/api/test">나만의 전통주 찾기</Link>
      </div>
      <div className="nav-bar">
        <Link to="/api/bar">전통주점</Link>
      </div>
      ...
      {!localStorage.getItem("token") ? (
        <div className="nav-login">
          <Link to="/api/kakaologin" className="logIn-button">
            로그인
          </Link>
        </div>
      ) : (
        <>
          <div className="nav-user">
            <Link to="/api/mypage" className="nav-mypage">
              마이페이지
            </Link>
          </div>
