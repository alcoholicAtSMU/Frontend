  ...
  const setNewMemberInfo = () => {
    ...
    axios
      ...
      .then((res) => {
        console.log(res);
        setModalState(false);
        navigate("/api/main", { replace: true }); // 로그인 완료 후 "/api/main"로 화면 전환
      })
      .catch((err) => {
  ...
  const kakaoLogin = (code: string | null, navigate: any) => {
    axios({
      ...
      .then((res) => {
        ...
        else navigate("/api/main", { replace: true }); // 로그인 완료 후 "/api/main"로 화면 전환
      })
      .catch((err) => {
        console.log("소셜 로그인 에러", err);
        navigate("/api/kakaologin", { replace: true }); // 로그인 실패 시 "/login"으로 화면 전환
      });
  };
